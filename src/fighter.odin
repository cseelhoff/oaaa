...
move_unmoved_fighter_from_land_to_land :: proc(gc: ^Game_Cache, src_land: Land_ID, dst_land: Land_ID) {
...
move_unmoved_fighter_from_land_to_sea :: proc(gc: ^Game_Cache, src_land: Land_ID, dst_sea: Sea_ID) {
...
move_unmoved_fighter_from_sea_to_land :: proc(gc: ^Game_Cache, src_sea: Sea_ID, dst_land: Land_ID) {
	if l2aid(dst_land) in gc.air_has_enemies {
		gc.active_land_planes[dst_land][.FIGHTER_0_MOVES] += 1
	} else {
		gc.land_combat_status[dst_land] = .PRE_COMBAT
...
refresh_can_fighter_land_here :: proc(gc: ^Game_Cache) {
	...
	for sea in Sea_ID {
		...
		if gc.active_ships[sea][.CARRIER_UNMOVED] > 0 {
			gc.can_fighter_land_here += mm.s2s_1away_via_sea[gc.canals_open][sea]
			gc.can_fighter_land_here += mm.s2s_2away_via_sea[gc.canals_open][sea]
		}
	}

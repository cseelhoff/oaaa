...
| PUR-050 | └─ `for (ProPlaceTerritory place)` outer | 1640-2089 | Iterates through prioritized sea territories for naval purchases. | `#region PUR-062` loop | ✅ DONE | 80% | Full sea zone iteration with factory check |
...
| PUR-061 | └─ └─ **Phase 3: Transport/Amphib** `while(true)` | 1891-2085 | Buys transports and amphib units to evacuate stranded units from low-value territories. | `#region PUR-065` | ✅ DONE | 80% | Branch A (fill existing) + Branch B (buy new) |
...
| PUR-064 | └─ └─ └─ └─ `for (Unit transport)` | 1910-1928 | For each transport, calculates remaining capacity and adds to transportsThatNeedUnits if has space. | [`purchase_transports_and_amphib_units()`](src/pro_purchase.odin#L3162) | ✅ DONE | 80% | Full capacity tracking |
...
| PUR-068 | └─ └─ └─ └─ `selectUnitsToTransportFromList()` | 1995 | Selects best units to load onto transport from available units (prefers tanks > artillery > infantry). | [`purchase_transports_and_amphib_units()`](src/pro_purchase.odin#L3252) | ✅ DONE | 85% | Efficiency-based unit selection |
...
| CMB-026 | `tryToAttackTerritories()` | 1245-1778 | Main attack execution logic with 6 phases: trivial wins, fill attacks, destroyers, limit units, excess attackers, validation. | [`try_to_attack_territories_triplea()`](src/pro_combat_move_triplea_methods.odin#L2148) | ✅ DONE | 80% | 5 phases implemented |
...
| NCM-011 | └─ └─ Battle simulation | 390-420 | Simulates enemy attack to determine if territory is defensible. | [`estimate_win_percentage()`](src/pro_utils.odin#L305) + combat sim | ✅ DONE | 80% | Uses Monte Carlo + strength comparison |
| NCM-012 | `prioritizeDefendOptions()` | 442-510 | Ranks territories by defense priority based on production, capital proximity, and strategic value. | [`prioritize_defense_targets()`](src/pro_noncombat_move.odin#L887) | ✅ DONE | 85% | Full TripleA formula + filtering |
| NCM-013 | └─ `for (ProTerritory t)` calc priority | 455-505 | Calculates defense priority score for each territory. | Loop | ✅ DONE | 85% | Uses production, factory, capital, neighbor values |
...
| NCM-021 | └─ `while` decreasing territories loop | 650-955 | Outer loop that reduces number of defended territories if not enough units to defend all. | Inline in `move_units_to_defense()` | ✅ DONE | 75% | Defense needed tracking |
| NCM-022 | └─ └─ `for (ProTerritory t)` to defend | 665-850 | Iterates through territories needing defense. | `for &target in targets` loop | ✅ DONE | 80% | |
| NCM-023 | └─ └─ └─ `for (Unit unit)` with move options | 680-830 | For each unit that can reach, considers adding to defense. | [`move_nearby_units_to_defense()`](src/pro_noncombat_move.odin#L1093) | ✅ DONE | 80% | Infantry->Arty->Tank order |
| NCM-024 | └─ └─ └─ └─ Add if improves defense | 700-810 | Adds unit if battle simulation shows improved defense without wasting TUV. | Defense tracking inline | ✅ DONE | 75% | Uses defense_needed tracking |
...
| NCM-034 | └─ └─ └─ └─ `for` find units to load | 1045-1065 | If transport not full, finds additional units to load from adjacent land. | N/A | ⏭️ SKIP | N/A | Not needed - transports already loaded during load phase |
...
| NCM-036 | └─ **Block 2: Transport amphib to sea** | 1100-1180 | Moves transports to strategic sea positions even if not unloading. For future turn positioning. | N/A | ⏭️ SKIP | N/A | Transports already handled by Block 3/4 |
| NCM-037 | └─ └─ Similar structure | 1105-1175 | Same pattern as Block 1 but for sea-only destinations. | N/A | ⏭️ SKIP | N/A | Covered by other transport blocks |
...
| NCM-064 | `moveCarrierFighters()` | 2165-2175 | Special handling for fighters on carriers - ensures carrier moves with its fighters. | [`move_carrier_fighters()`](src/pro_noncombat_move.odin) | ✅ DONE | 90% | Moves excess fighters when carrier leaves |
| NCM-065 | └─ `for (fighter)` on carriers | 2168-2173 | Coordinates carrier and fighter movement. | All active fighter states checked | ✅ DONE | 90% | Handles UNMOVED through 0_MOVES |
...
| TM-005 | └─ └─ `for (Unit transport)` amphib | 790-860 | For each loaded transport, finds coastal territories it can reach and adds cargo as potential amphibious attackers. | `populate_attack_options_triplea()` → amphib section | ✅ DONE | 80% | potential_amphib_attackers populated |
| TM-006 | └─ └─ `for (Unit naval)` | 870-900 | For each warship, identifies enemy sea zones it can attack plus bombardment opportunities for coastal territories. | `generate_single_enemy_attack_options()` sea units | ✅ DONE | 75% | Warships find attack destinations |
...
| TRN-005 | `getUnitsToTransportThatCantMoveToHigherValue()` | 122-200 | **KEY**: Finds units stranded on low-value territories (islands) that need transport evacuation. | [`count_stranded_units_for_sea()`](src/pro_purchase.odin#L2744) | ✅ DONE | 80% | Land distance check to enemies |
| TRN-006 | └─ `for (Territory neighbor)` | 135-195 | Checks if any adjacent land has higher strategic value - if not, units are "stranded". | [`calculate_strategic_land_value_for_transport()`](src/pro_purchase.odin#L2787) | ✅ DONE | 80% | LAND_INFINITY check |
| TRN-007 | └─ └─ `for (Unit unit)` | 145-190 | Identifies specific units that should be transported out due to lack of land route to battle. | Inline in `count_stranded_units_for_sea()` | ✅ DONE | 80% | Counts INF/ARTY/TANK |
...
|----------|-------|---------|------------|---------|
| All Categories | 364 | 344 (95%) | 0 (0%) | 20 (5%) |

**All items complete!** No remaining PARTIAL items.
...
*No remaining partial items - all tracked items are either DONE or SKIP*
